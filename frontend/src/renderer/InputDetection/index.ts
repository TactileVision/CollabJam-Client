...
export const createInputDetection = (config: Partial<InputDetectionConfig> & Pick<InputDetectionConfig, 'onInput'>): InputDetection => {
  ...
  const { buttonThreshold, axesThreshold, onInput } = configWithDefaults
  const adapters = configWithDefaults.adapters.map(adapterFn => adapterFn({ buttonThreshold, axesThreshold, onInput }));
