    ...
    <template v-slot:default="{ item, index: groupIndex }">
      <v-expansion-panels :key="groupIndex">
        <v-expansion-panel :elevation="'0'">
          <v-expansion-panel-title :class="groupIndex === selectedPrefixgroupId ? 'text-primary' : ''">
            {{item.prefix}}
          ...
          <v-expansion-panel-text>
            ...
            >
              ...
              >
                ...
                <v-list-item-title @click="selectTacton(tacton, groupIndex)">
                  {{tacton.metadata.name}}
                </v-list-item-title>

                <v-list-item-subtitle class="py-2"  @click="selectTacton(tacton, groupIndex)">
                  <v-chip size="small" v-show="!hasMetadata(tacton)">
                    <v-icon size="" icon="mdi-file-document-remove-outline"></v-icon>
                    No details
                  </v-chip>
                  <v-chip v-show="hasMetadata(tacton)">
                    <v-icon size="" icon="mdi-file-document-check-outline"></v-icon>
                    details
                  </v-chip>
          ...
          <v-row dense>
            ...
            >
              <v-text-field
                  label="RecordDate"
                  :model-value="
                  Intl.DateTimeFormat('default', {dateStyle: 'long'}).format(new Date(optionsTacton!.metadata.recordDate))"
                  variant="underlined"
                  readonly
              ></v-text-field>
            </v-col>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      rules: {
        ...
        charLimit: (value: string) => value.length <= charLimit || `Input is limited to ${charLimit} characters`
      },
  ...
  methods: {
    ...
    hasMetadata(tacton: Tacton) {
      // TODO true if tacton has description | customTag | bodyTag
      return false;
    },
