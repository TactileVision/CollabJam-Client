...
export default defineComponent({
  ...
  methods: {
    resizeScreen() {
       ...
       */
      const newWidth =
        document.getElementById("TactonGraphWrapper")!.clientWidth;
      const newHeight = document.getElementById(
        "TactonGraphWrapperHeight",
      )!.clientHeight;
      // window.innerHeight -
          ...
          this.blocksByChannel.forEach((blocks, channelId) => {
            ...
            if (block && !waiting[channelId]) {
              block.instructions.push({
                wait: { miliseconds: i.wait.miliseconds },
              });
      ...
      for (let i = 0; i < this.numberOfOutputs; i++) {
        ...
        for (const { instructions, startMs } of blocks) {
          ...
          for (let index = 0; index < instructions.length; index += 1) {
            ...
            if (isInstructionSetParameter(instruction)) {
              const i = instruction as InstructionSetParameter;
              if (i.setParameter.intensity === 0) continue;
              ...
              while (nextInstruction && isInstructionWait(nextInstruction)) {
                const ni = nextInstruction as InstructionWait;
                timeBetweenInstructionsMs += ni.wait.miliseconds;
        ...
        blocks
          ...
          .forEach((block) => {
            ...
            block.instructions.forEach((instruction) => {
              let duration = 0;
              if (isInstructionWait(instruction)) {
                duration = (instruction as InstructionWait).wait.miliseconds;
              }
          ...
          if (entry.endMs >= nextEntry.startMs) {
            if (isInstructionWait(entry.instruction)) {
              if (isInstructionWait(nextEntry.instruction)) {
                (entry.instruction as InstructionWait).wait.miliseconds += (
                  nextEntry.instruction as InstructionWait
                ).wait.miliseconds;
              } else {
                (entry.instruction as InstructionWait).wait.miliseconds =
              ...
              if (
                ...
                (entry.instruction as InstructionSetParameter).setParameter
                  .intensity ===
                  (nextEntry.instruction as InstructionSetParameter)
                    .setParameter.intensity
              ) {
                (
