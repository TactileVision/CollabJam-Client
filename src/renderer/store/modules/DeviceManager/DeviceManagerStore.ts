...
export type Mutations<S = State> = {
  [DeviceMutations.ADD_CONNECTED_DISPLAY](
    state: S,
    device: TactileDisplay,
  ): void;
  [DeviceMutations.REMOVE_DISPLAY](state: S, uuid: string): void;
  [DeviceMutations.UPDATE_CONNECTED_DISPLAY_LIST](
    state: S,
    deviceList: TactileDisplay[],
  ): void;
  [DeviceMutations.UPDATE_CONNECTED_DISPLAY](
    state: S,
    item: { index: number; device: TactileDisplay },
  ): void;
  [DeviceMutations.ADD_SCANNED_PERIPHERAL](
    state: S,
    device: PeripheralInformation,
  ): void;
  [DeviceMutations.UPDATE_SCANNED_PERIPHERAL_LIST](
    state: S,
    deviceList: PeripheralInformation[],
  ): void;
  [DeviceMutations.UPDATE_FREQ_INFORMATION](
    state: S,
    info: { uuid: string; freqInfo: FrequencyInformation },
  ): void;
};
...
export const actions: ActionTree<State, RootState> & Actions = {
  [DeviceManagerStoreActionTypes.discoveredPeripheral](
    { commit },
    newDevice: PeripheralInformation,
  ) {
  ...
  [DeviceManagerStoreActionTypes.addConnectedDisplay](
    { commit },
    newDevice: TactileDisplay,
  ) {
    if (
      state.connectedTactileDisplays.some(
        (device) => device.info.id == newDevice.info.id,
      )
    )
      return;
  ...
  [DeviceManagerStoreActionTypes.updatePeripheralStatus](
    { commit },
    modifiedDevice: TactileDisplay,
  ) {
    const index = state.connectedTactileDisplays.findIndex(
      (device) => device.info.id === modifiedDevice.info.id,
    );
    //no device found
    if (index == -1) return;

    commit(DeviceMutations.UPDATE_CONNECTED_DISPLAY, {
      index: index,
  ...
  [DeviceManagerStoreActionTypes.setNumberOfOutputs](
    ...
    const index = state.connectedTactileDisplays.findIndex(
      (device) => device.info.id === payload.deviceId,
    );
    //no device found
    if (index == -1) return;
    ...
    commit(DeviceMutations.UPDATE_CONNECTED_DISPLAY, {
      index: index,
      device: { ...state.connectedTactileDisplays[index] },
    });
  },
  [DeviceManagerStoreActionTypes.setFreqAvailability](
    { commit },
  ...
  [DeviceManagerStoreActionTypes.setFreqInfo](
    ...
    commit(DeviceMutations.UPDATE_FREQ_INFORMATION, {
      uuid: payload.deviceId,
      freqInfo: payload.freqInfo,
    });
  },
};
