  ...
  selectedProfiles: [
    {
      device: {
        ...
        name: "©Microsoft Corporation Controller (STANDARD GAMEPAD Vendor: 045e Product: 028e)",
        index: 0,
      } as GamepadDevice,
      profileIndex: 0,
    },
...
export type Mutations<S = State> = {
  [PlayGroundMutations.BULK_GRID_UPDATE](
    state: S,
    deviceBindings: InputProfile[],
  ): void;
  [PlayGroundMutations.UPDATE_GRID_ITEM](
    state: S,
    payload: {
      index: number;
  ...
  [PlayGroundMutations.ADD_ITEM_TO_GRID](
    state: S,
    payload: { profile: StateProfile; binding: StateInputBinding },
  ): void;
  [PlayGroundMutations.DELETE_ITEM_FROM_GRID](
    state: S,
    payload: { profileUid: string; uid: string },
  ): void;
  [PlayGroundMutations.UPDATE_GLOBAL_INTENSITY](
    state: S,
    intensity: number,
  ): void;
  [PlayGroundMutations.UPDATE_EDIT_MDOE](state: S, edditModeOn: boolean): void;
  [PlayGroundMutations.UPDATE_PROFILE](
    state: S,
    payload: { device: InputDevice; profileUid: string },
  ): void;
};
...
export const mutations: MutationTree<State> & Mutations = {
  ...
  [PlayGroundMutations.UPDATE_GRID_ITEM](state, payload) {
    const profileIndex = state.profiles.findIndex(
      (profile) => profile.uid === payload.profile.uid,
    );
  ...
  [PlayGroundMutations.DELETE_ITEM_FROM_GRID](state, payload) {
    const deviceIndex = state.profiles.findIndex(
      (profile) => profile.uid === payload.profileUid,
    );
    const index = state.profiles[deviceIndex].bindings.findIndex(
      (binding) => binding.uid === payload.uid,
    );
    if (index != -1) state.profiles[deviceIndex].bindings.splice(index, 1);
  },
...
export interface Actions {
  [PlayGroundActionTypes.activateKey](
    ...
    payload: {
      ...
      value: number;
      wasActive: boolean;
    }, // Obsolete in here but left as an example
  ): void;
  ...
  [PlayGroundActionTypes.addButtonToGrid](
    { commit }: AugmentedActionContext,
    payload: {
      binding: Omit<InputBinding, "uid" | "position">;
      profile: StateProfile;
    }, // Obsolete in here but left as an example
  ): void;
...
export const actions: ActionTree<State, RootState> & Actions = {
  [PlayGroundActionTypes.activateKey](
    ...
    const index = payload.profile.bindings.findIndex((binding) =>
      compareInputs(binding.inputs[0], payload.input),
    );
    if (index == -1) return;
    ...
    const newBinding = {
      ...binding,
      activeTriggers: binding.activeTriggers + 1,
    };
    ...
    if (!payload.wasActive)
      commit(PlayGroundMutations.UPDATE_GRID_ITEM, {
        index,
  ...
  [PlayGroundActionTypes.deactivateKey](
    { commit },
    payload: { profile: StateProfile; input: UserInput },
  ) {
    const index = payload.profile.bindings.findIndex((binding) =>
      compareInputs(binding.inputs[0], payload.input),
    );
  ...
  [PlayGroundActionTypes.addButtonToGrid](
     ...
     * */
    const usedPositions = state.profiles
    ...
    const inputBinding = {
      uid,
      position: { h: 1, w: 1, ...space },
      ...payload.binding,
    };
    //save updated keyBoard inside of config
    // get profile by device -> add profile uid to ipc call
    window.api.send(IPC_CHANNELS.main.saveKeyBoardButton, {
      profileUid: payload.profile.uid,
      binding: inputBinding,
    });
    //save it in store
    commit(PlayGroundMutations.ADD_ITEM_TO_GRID, {
      profile: payload.profile,
      binding: { activeTriggers: 0, ...inputBinding },
    });
  },
  [PlayGroundActionTypes.updateKeyButton](
    { commit },
    ...
    const index = state.profiles[profileIndex].bindings.findIndex(
      (binding) => binding.uid === payload.id,
    );
    if (index == -1) return;

    const { activeTriggers, ...oldBinding } =
      state.profiles[profileIndex].bindings[index];
    ...
    commit(PlayGroundMutations.UPDATE_GRID_ITEM, {
      ...
      binding: { activeTriggers, ...binding },
    });
  },
...
export type Getters = {
  getKeyButton(
    ...
    id: string,
  ) => { binding: StateInputBinding; profile: StateProfile } | undefined;
  getProfileByDevice(
    state: State,
  ): (device: InputDevice) => StateProfile | undefined;
...
export const getters: GetterTree<State, RootState> & Getters = {
  getKeyButton: (state) => (id) => {
    for (const profile of state.profiles) {
      for (const binding of profile.bindings) {
        if (binding.uid === id) return { binding, profile };
      }
    }
  },
  getProfileByDevice: (state) => (device) => {
    const selection = state.selectedProfiles.find(({ device: profileDevice }) =>
      compareDevices(profileDevice, device),
    );
  ...
  isActiveKey: (state) => (id) => {
    const binding = state.profiles
      .flatMap((profile) => profile.bindings)
      .find((binding) => binding.uid === id);
