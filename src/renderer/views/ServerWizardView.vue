  ...
  <v-dialog v-model="dialog" width="800" height="350" persistent>
    ...
    <v-container class="h-100" v-if="setupState == SetupState.ADD_SERVER">
      ...
      >
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn text="Cancel Connection" @click="cancelConnection"></v-btn>
        </v-card-actions>
      </v-card>
      <!--Connection failed-->
      <v-card
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      urlRules: [
        (url: string) => {
          if (url) {
            const duplicateServerName: string =
              this.checkForExistingServerUrl(url);
  ...
  methods: {
    ...
    saveServer(): void {
      const newServer: Server = {
        ...
        name:
          this.name !== ""
            ? this.name
            : `Server${this.serversFromJSON.length + 1}`,
        port: Number.parseInt(this.port),
      };
      this.serversFromJSON.push(newServer);
      localStorage.setItem(
        LocalStorageKey.SERVER_LIST,
        JSON.stringify(this.serversFromJSON),
      );
    ...
    checkForExistingServerName(name: string): boolean {
      return this.serversToDisplay.some((server) => {
  ...
  mounted() {
    ...
    const serverFromEnv = JSON.parse(
      import.meta.env.VITE_COLLABJAM_SERVERS || "[]",
    );
