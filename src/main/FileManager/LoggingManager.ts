...
class LoggingManager {
  ...
  constructor() {
    ...
    this.pathSettings = path.join(
      userDataPath,
      "logging",
      `log_${date.getDate()}_${date.getMonth() + 1}_${date.getFullYear()}.txt`,
    );
    const readSuccessfully = this.checkIfFileExist();

    if (readSuccessfully == false) this.initLoggingFile();
  }
  ...
  writeLog(level: LoggingLevel, type: string, latency: number) {
    fs.appendFile(
      this.pathSettings,
      JSON.stringify({ type: type, latency: latency + "ms", level: level }) +
        "\r\n",
      (err) => {
        if (err) {
          console.log(err);
