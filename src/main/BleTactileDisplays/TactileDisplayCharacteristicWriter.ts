...
import { tactileDisplayService } from "@/main/BleConnection/BleServices";
 ...
 */
export async function pingDisplayViaNode(
  device: Peripheral,
  durationMs: number,
) {
...
export const writeAmplitudeBuffer = (
  ...
  if (service !== undefined) {
    const characteristic = service.characteristics.find(
      (characteristic) =>
        characteristic.uuid ===
        tactileDisplayService.characteristics!.amplitudeValues.uuid,
    );
...
export const writeAmplitudeBuffers = (
  devices: Peripheral[],
  taskList: TactileTask[],
) => {
...
function writeAmplitudeBufferCharacteristic(
  buffer: Buffer,
  characteristic: Characteristic,
) {
  console.log("writing amplitude");
...
export function writeFrequencyBuffer(
  device: Peripheral,
  taskList: SetFrequencyTask[],
) {
  ...
  if (service !== undefined) {
    const characteristic = service.characteristics.find(
      (characteristic) =>
        characteristic.uuid ===
        tactileDisplayService.characteristics!.frequencyValues.uuid,
    );
    if (characteristic !== undefined) {
...
function writeFrequencyBuffercharacteristic(
  buffer: Buffer,
  characteristic: Characteristic,
) {
