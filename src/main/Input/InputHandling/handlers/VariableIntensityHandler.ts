...
const isSetIntensityAction = (
  action: TactileAction,
): action is SetIntensityAction => {
        ...
        Object.entries(groupedActions).forEach(([name, channels]) => {
          ...
          channels.forEach((c) => {
            ...
            if (binding.activeTriggers == 0 && li.get(c) != 0) {
              ...
              int > storedIntensity + sendThreshold ||
              int < storedIntensity - sendThreshold
            ) {
