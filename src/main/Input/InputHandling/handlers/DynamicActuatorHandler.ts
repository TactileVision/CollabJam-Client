...
const isDynamicActuatorAction = (
  action: TactileAction,
): action is DynamicActuatorAction => {
  ...
  return {
    onInput: ({ binding, value, globalIntensity }) => {
      ...
      binding.actions.filter(isDynamicActuatorAction).forEach((action) => {
        const configsWithIndex = action.actuators.map((config, index) => ({
          config,
          index,
        }));
        const active = configsWithIndex
          .filter(
            ({ config }) => config.minValue < value && config.maxValue >= value,
          )
          .map(({ index }) => index);
        const inactive = configsWithIndex
          .filter(({ index }) => !active.includes(index))
          .map(({ index }) => index);

        const stillActiveChannels = new Set(
          active.flatMap(
            (configIndex) => action.actuators[configIndex].channels,
          ),
        );
        if (active.length > 0) {
          active.forEach((configIndex) => {
            ...
            const intensity =
              resolveIntensity(config.intensity) * globalIntensity;
        ...
        if (inactive.length > 0) {
          ...
          const maybeInactiveChannels = inactive.flatMap(
            (configIndex) => action.actuators[configIndex].channels,
          );
          const inactiveChannels = new Set(
            maybeInactiveChannels.filter(
              (channel) =>
                activeChannels.has(channel) &&
                !stillActiveChannels.has(channel),
            ),
          );
