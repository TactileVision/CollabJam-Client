...
export const debouncedHandling = (
  ...
  if (instructions.length > 0) {
    ...
    const uniqueChannels = [
      ...new Set(instructions.map((item) => item.channels).flat()),
    ]; // [ 'A', 'B']
    ...
    uniqueChannels.forEach((c) => {
      const lastForInstructionChannel = instructions
        .filter((instruction) => {
    ...
    const uniqueIntensities = [
      ...new Set(lastInstructions.map((item) => item.intensity)),
    ]; // [ 'A', 'B']
